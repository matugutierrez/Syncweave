...
export function BlockHandle({ blockId, onDelete, onDuplicate }: BlockHandleProps) {
  const [open, setOpen] = useState(false)

  return (
    <div className="group absolute -left-10 top-0 flex h-full items-center">
      <button
        onClick={() => setOpen(!open)}
        className="flex h-6 w-6 items-center justify-center rounded text-xs opacity-0 transition-all group-hover:opacity-100 hover:bg-[var(--bg-hover)]"
      ...
      {open && (
        <div
          className="animate-fade-in absolute left-6 top-0 z-30 w-36 rounded-xl border py-1 shadow-xl"
          style={{
